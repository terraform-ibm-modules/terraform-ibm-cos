...
module "cos_fscloud" {
  ...
  # Create one regional bucket, encrypted with the KMS root key
  bucket_configs = [{
    access_tags              = var.access_tags
    bucket_name              = "${var.prefix}-bucket"
    kms_key_crn              = module.kms_keys.keys["${local.key_ring_name}.${local.bucket_key_name}"].crn
...
module "backup_vault" {
  ...
  kms_key_crn              = module.kms_keys.keys["${local.key_ring_name}.${local.vault_key_name}"].crn
}
