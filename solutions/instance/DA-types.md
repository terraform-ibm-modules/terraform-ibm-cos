...
In the configuration, specify the name of the resource key, whether HMAC credentials should be included, the role of the key, the endpoint type, and an optional Service ID CRN to create with a Service ID.
...
- `endpoint` (optional, default = `private`): The Cloud Object Storage endpoint that the resource key uses. Allowed values are `public` and `private`.
