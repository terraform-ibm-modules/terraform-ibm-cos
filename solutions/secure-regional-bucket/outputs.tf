...
output "s3_endpoint_direct" {
  ...
  value       = try(module.cos.buckets[var.bucket_name].s3_endpoint_direct, "")
}

output "s3_endpoint_private" {
  description = "The s3 private endpoint of the created bucket."
  value       = try(module.cos.buckets[var.bucket_name].s3_endpoint_private, "")
}

output "bucket_name" {
  description = "The name of the bucket that was created. Includes the optional suffix if enabled."
  value       = try(module.cos.buckets[var.bucket_name].bucket_name, "")
}
