...
| <a name="input_public_access_role"></a> [public\_access\_role](#input\_public\_access\_role) | IAM role to grant to the Public Access access group for the COS bucket. Only applicable when `allow_public_access_to_bucket` is `true` and `create_cos_bucket` is `true`. | `string` | `"ObjectReader"` | no |
