...
| <a name="input_cos_plan"></a> [cos\_plan](#input\_cos\_plan) | The plan to use when Object Storage instances are created. Possible values: `standard`, `cos-one-rate-plan`. Applies only if `create_cos_instance` is true. The standard plan includes a Free Tier of 5GB free storage for 12 months. To access the Free Tier select 'smart' as your bucket storage class. | `string` | `"standard"` | no |
